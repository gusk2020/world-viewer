...
export const SEASONAL_TIME_AXIS = {
  ...
  /** Solar declination at one orbital phase, for an obliquity in degrees. */
...
export const SEASON_PARAMETERS = {
  ...
  // How many harmonics of the year the table keeps. Four is measured: it
  // holds the response to about 0.5 C everywhere, including the sharply
  // non-sinusoidal polar forcing, at 32 KB for 512 rows.
  harmonics: 4,
};
...
export function buildSeasonalTemperatureTable({
  ...
  for (let y = 0; y < rows; y++) {
    ...
    for (let s = 0; s < phaseSamples; s++) {
      const dec = SEASONAL_TIME_AXIS.declinationRad(s / phaseSamples, tilt);
      forcing[s] = dailyMeanInsolationFactor(latRad, dec);
      mean += forcing[s];
    }
    mean /= phaseSamples;
